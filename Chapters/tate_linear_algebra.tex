...
\end{definition}
%\begin{proposition}\label{prop:completeness-is-closure}
...
%\]
%is exact. 
%completion map $V \to \widehat{V}$.
%\end{enumerate}
%\end{proposition}
%\begin{proof}
%that exactness of (a) follows from exactness of the sequences
%\[
%0 \to W/(W \cap U) \to V/U \to V/(W + U) \to 0
%\]
%to \widehat{V/W}$ is closed.
%\end{proof}
%def:linear_compactness} can be dropped.
%\begin{proposition}\label{prop:complete-linear-compactness-closed}
%$W$ is linearly compact.
%\end{proposition}
%\begin{proof}
%image of $W$ under the completion map $V \to \widehat{V}$ is the inverse limit
%\[
%\varprojlim_{U\in \mathscr{U}} (W + U)/U
%\]
%which is the completion of $W$. Thus, by 
%\end{proof}
